import { useSubcategoriesMonthyExpense } from '../queries/subcategories/useSubcategoriesMonthlyExpense'
import { Cell, Pie, PieChart, ResponsiveContainer } from 'recharts'
import { numberToCurrency } from '../utils/number-to-currency'
import { CHART_RAMP } from '../utils/chart-colors'

type SubcategoryExpensesProps = {
  month?: number
  year?: number
}

export function SubcategoryExpenses({ month, year }: SubcategoryExpensesProps = {}) {
  const { data: expenseSubcategories, isLoading } = useSubcategoriesMonthyExpense(
  ...
  const entries = expenseSubcategories ? Object.entries(expenseSubcategories) : []
  ...
  return (
    <div className="rounded-[10px] border border-border bg-card p-5">
      <h2 className="mb-4 text-[14px] font-semibold">Expenses by subcategory</h2>
      {isLoading ? (
